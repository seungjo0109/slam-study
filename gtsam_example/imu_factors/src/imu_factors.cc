...
{
    string input_data;
    // input_data = gtsam::findExampleDataFile("imuAndGPSdata.csv");
    if(argc < 2){
        cerr << "Input file error: enter input data filename";
        return 1;
    }
    else{
        input_data = argv[1];
    ...
    ifstream file(input_data.c_str());
    string value;
    ...
    noiseModel::Diagonal::shared_ptr pose_noise_model = noiseModel::Diagonal::Sigmas((Vector(6) << 0.01, 0.01, 0.01, 0.5, 0.5, 0.5).finished());
    ...
    /* Keep track of the total error over the entire run for a simple performed metric. */
