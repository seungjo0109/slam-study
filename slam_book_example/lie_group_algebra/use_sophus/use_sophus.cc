...
int main()
{
    ...
    std::cout << "so3 hat vee= " << Sohpus::SO3d::vee(Sophus::SO3d::hat(so3)).transpose() << std::endl;

    // Update by perturbation model
    Eigen::Vector3d update_so3(1e-4,0,0);   // this is a small update
    Sohpus::SO3d SO3_updated = Sophus::SO3d::exp(update_so3) * SO3_R;
    std::cout << "SO3 updated = \n" << SO3_udpated.matrix() << std::endl;
    ...
    Sohpus::SE3d SE3_updated = Sophus::SE3d::exp(update_se3) * SE3_Rt;
