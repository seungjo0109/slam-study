...
std::string groudtruth_file = "../example/groundtruth.txt";
...
int main()
{
    TrajectoryType groudtruth = ReadTrajectory(groudtruth_file);
    ...
    DrawTrajectory(groudtruth, estimated);
...
TrajectoryType ReadTrajectory(const std::string &path) {
    ...
    if (!fin) {
        std::cerr << "trajectory " << path << " not found.\n";
        return trajectory;
    }
